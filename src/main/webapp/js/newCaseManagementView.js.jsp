...
<%
    ...
    if ( tabViewProp == null ) {
      ...
      // Load CPP preferences for this provider                                                        
      CppPreferencesUIBean cppPrefs = new CppPreferencesUIBean(curUser_no);                            
      cppPrefs.loadValues();                                                                           
      boolean cppEnabled = "on".equals(cppPrefs.getEnable());                                          
                                                                                                       
      // Show/hide flags for left nav sections (sections without CPP prefs are always shown)           
      // When CPP enabled: "SHOW" = visible, empty string = hidden                                     
      boolean showPreventions = !cppEnabled || "SHOW".equals(cppPrefs.getPreventionsDisplay());        
      boolean showDx = !cppEnabled || "SHOW".equals(cppPrefs.getDxRegistryDisplay());                  
      boolean showForms = !cppEnabled || "SHOW".equals(cppPrefs.getFormsDisplay());                    
      boolean showEforms = !cppEnabled || "SHOW".equals(cppPrefs.getEformsDisplay());                  
      boolean showDocs = !cppEnabled || "SHOW".equals(cppPrefs.getDocumentsDisplay());                 
      boolean showLabs = !cppEnabled || "SHOW".equals(cppPrefs.getLabsDisplay());                      
      boolean showMeasurements = !cppEnabled || "SHOW".equals(cppPrefs.getMeasurementsDisplay());      
      boolean showConsultation = !cppEnabled || "SHOW".equals(cppPrefs.getConsultationsDisplay());     
      boolean showHRM = !cppEnabled || "SHOW".equals(cppPrefs.getHrmDisplay());                        
                                                                                                       
      // Show/hide flags for right nav sections                                                        
      boolean showAllergies = !cppEnabled || "SHOW".equals(cppPrefs.getAllergiesDisplay());            
      boolean showRx = !cppEnabled || "SHOW".equals(cppPrefs.getMedicationsDisplay());                 
      boolean showOMeds = !cppEnabled || "SHOW".equals(cppPrefs.getOtherMedsDisplay());                
      boolean showRiskFactors = !cppEnabled || "SHOW".equals(cppPrefs.getRiskFactorsDisplay());        
      boolean showFamHistory = !cppEnabled || "SHOW".equals(cppPrefs.getFamilyHxDisplay());            
      boolean showUnresolvedIssues = !cppEnabled || "SHOW".equals(cppPrefs.getUnresolvedIssuesDisplay());                                                
      boolean showResolvedIssues = !cppEnabled || "SHOW".equals(cppPrefs.getResolvedIssuesDisplay());  
      boolean showEpisode = !cppEnabled || "SHOW".equals(cppPrefs.getEpisodesDisplay());               
                                                                                                       
      // Position preferences for CPP boxes                                                            
      String socialHxPos = cppPrefs.getSocialHxPosition() != null ? cppPrefs.getSocialHxPosition() : "R1I1";                                                                                              
      String medicalHxPos = cppPrefs.getMedicalHxPosition() != null ? cppPrefs.getMedicalHxPosition() : "R1I2";                                                                                             
      String ongoingConcernsPos = cppPrefs.getOngoingConcernsPosition() != null ? cppPrefs.getOngoingConcernsPosition() : "R2I1";                                                      
      String remindersPos = cppPrefs.getRemindersPosition() != null ? cppPrefs.getRemindersPosition() : "R2I2";                                                                                             
...
function navBarLoader() {
   ...
   $("rightNavBar").style.height = "660px";
    ...
    this.load = function() {
            ...
            var leftNavBar = [];
            var leftNavBarTitles = [];
    <% if (showPreventions) { %>                                                                         
	  leftNavBar.push(ctx + "/oscarEncounter/displayPrevention.do?hC=" + Colour.prevention);               
	  leftNavBarTitles.push("preventions");                                                                
  <% } %>                                                                                              
	  leftNavBar.push(ctx + "/oscarEncounter/displayTickler.do?hC=" + Colour.tickler);                     
	  leftNavBarTitles.push("tickler");                                                                    
  <% if (showDx) { %>                                                                                  
	  leftNavBar.push(ctx + "/oscarEncounter/displayDisease.do?hC=" + Colour.disease);                     
	  leftNavBarTitles.push("Dx");                                                                         
  <% } %>                                                                                              
  <% if (showForms) { %>                                                                               
	  leftNavBar.push(ctx + "/oscarEncounter/displayForms.do?hC=" + Colour.forms);                         
	  leftNavBarTitles.push("forms");                                                                      
  <% } %>                                                                                              
  <% if (showEforms) { %>                                                                              
	  leftNavBar.push(ctx + "/oscarEncounter/displayEForms.do?hC=" + Colour.eForms);                       
	  leftNavBarTitles.push("eforms");                                                                     
  <% } %>                                                                                              
  <% if (showDocs) { %>                                                                                
	  leftNavBar.push(ctx + "/oscarEncounter/displayDocuments.do?hC=" + Colour.documents);                 
	  leftNavBarTitles.push("docs");                                                                       
  <% } %>                                                                                              
  <% if (showLabs) { %>                                                                                
	  leftNavBar.push(ctx + "/oscarEncounter/displayLabs.do?hC=" + Colour.labs);                           
	  leftNavBarTitles.push("labs");                                                                       
  <% } %>                                                                                              
	  leftNavBar.push(ctx + "/oscarEncounter/displayMessages.do?hC=" + Colour.messages);                   
	  leftNavBarTitles.push("msgs");                                                                       
  <% if (showMeasurements) { %>                                                                        
	  leftNavBar.push(ctx + "/oscarEncounter/displayMeasurements.do?hC=" + Colour.measurements);           
	  leftNavBarTitles.push("measurements");                                                               
  <% } %>                                                                                              
  <% if (showConsultation) { %>                                                                        
	  leftNavBar.push(ctx + "/oscarEncounter/displayConsultation.do?hC=" + Colour.consultation);           
	  leftNavBarTitles.push("consultation");                                                               
  <% } %>                                                                                              
  <% if (showHRM) { %>                                                                                 
	  leftNavBar.push(ctx + "/oscarEncounter/displayHRM.do?hC=");                                          
	  leftNavBarTitles.push("HRM");                                                                        
  <% } %>                                                                                              
	  leftNavBar.push(ctx + "/eaaps/displayEctEaaps.do?hC=");                                              
	  leftNavBarTitles.push("eams");                                                                       
	  leftNavBar.push(ctx + "/oscarEncounter/displayEconsultation.do?hC=");                                
	  leftNavBarTitles.push("eConsult"); 
	  ...
	  var rightNavBar = [];                                                                                
	  var rightNavBarTitles = [];                                                                          
																										   
	  <% if (showAllergies) { %>                                                                           
		  rightNavBar.push(ctx + "/oscarEncounter/displayAllergy.do?hC=" + Colour.allergy);                    
		  rightNavBarTitles.push("allergies");                                                                 
	  <% } %>                                                                                              
	  <% if (showRx) { %>                                                                                  
		  rightNavBar.push(ctx + "/oscarEncounter/displayRx.do?hC=" + Colour.rx + "&numToDisplay=12");         
		  rightNavBarTitles.push("Rx");                                                                        
	  <% } %>                                                                                              
	  <% if (showOMeds) { %>                                                                               
		  rightNavBar.push(ctx + "/CaseManagementView.do?hc=" + Colour.omed + "&method=listNotes&providerNo=" +
		   providerNo + "&demographicNo=" + demographicNo + "&issue_code=OMeds&title=" + oMedsLabel +          
		  "&cmd=OMeds" + "&appointment_no="+appointmentNo);                                                    
		  rightNavBarTitles.push("OMeds");                                                                     
	  <% } %>                                                                                              
	  <% if (showRiskFactors) { %>                                                                         
		  rightNavBar.push(ctx + "/CaseManagementView.do?hc=" + Colour.riskFactors +                           
		  "&method=listNotes&providerNo=" + providerNo + "&demographicNo=" + demographicNo +                   
		  "&issue_code=RiskFactors&title=" + riskFactorsLabel + "&cmd=RiskFactors"+                            
		  "&appointment_no="+appointmentNo);                                                                   
		  rightNavBarTitles.push("RiskFactors");                                                               
	  <% } %>                                                                                              
	  <% if (showFamHistory) { %>                                                                          
		  rightNavBar.push(ctx + "/CaseManagementView.do?hc=" + Colour.familyHistory +                         
		  "&method=listNotes&providerNo=" + providerNo + "&demographicNo=" + demographicNo +                   
		  "&issue_code=FamHistory&title=" + famHistoryLabel + "&cmd=FamHistory"+                               
		  "&appointment_no="+appointmentNo);                                                                   
		  rightNavBarTitles.push("FamHistory");                                                                
	  <% } %>                                                                                              
	  <% if (showUnresolvedIssues) { %>                                                                    
		  rightNavBar.push(ctx + "/oscarEncounter/displayIssues.do?hC=" + Colour.unresolvedIssues);            
		  rightNavBarTitles.push("unresolvedIssues");                                                          
	  <% } %>                                                                                              
	  <% if (showResolvedIssues) { %>                                                                      
		  rightNavBar.push(ctx + "/oscarEncounter/displayResolvedIssues.do?hC=" + Colour.resolvedIssues);      
		  rightNavBarTitles.push("resolvedIssues");                                                            
	  <% } %>                                                                                              
		  rightNavBar.push(ctx + "/oscarEncounter/displayDecisionSupportAlerts.do?providerNo=" + providerNo +  
		  "&demographicNo=" + demographicNo);                                                                  
		  rightNavBarTitles.push("Guidelines");                                                                
		  rightNavBar.push(ctx + "/oscarPrevention/preventionRecommendations.jsp?demographic_no="+             
		  demographicNo);                                                                                      
		  rightNavBarTitles.push("PreventionDS");                                                              
	  <% if (showEpisode) { %>                                                                             
		  rightNavBar.push(ctx + "/oscarEncounter/displayEpisodes.do?hC=" + Colour.episode);                   
		  rightNavBarTitles.push("episode");                                                                   
	  <% } %>                                                                                              
		  rightNavBar.push(ctx + "/oscarEncounter/displayPregnancies.do?hC="+ Colour.episode);                 
		  rightNavBarTitles.push("pregnancy");                                                                 
		  rightNavBar.push(ctx + "/oscarEncounter/displayContacts.do?hC=" + Colour.contacts);                  
		  rightNavBarTitles.push("contacts"); 
